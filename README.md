...
- **Voter count should be odd** (Raft quorum) and capped at the number of AZs with
  ...
  in 3 AZs is fine, but 5 voters in a 3-AZ region — which pre-4.2.0 just packed as
...
| `monte_carlo` | reader bundle¹ | `INSERT` on `llm_inputs`/`llm_batches`/`conversation_eval_scores`/`span_eval_scores`/`conversation_cluster_assignments` | Monte Carlo (data-source monitoring + agent observability) | always |
