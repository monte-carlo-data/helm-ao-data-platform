...
> **Telemetry retention** is controlled by `clickhouse.ttlDays` (default 30 days), covering the raw traces, the trace-id timestamp index, the normalized spans, and their derived annotations (`conversation_eval_scores`, `span_eval_scores`, `conversation_cluster_assignments`). Unlike the system-log TTLs above, the schema migration Job re-applies this on every install and upgrade (`ALTER TABLE … MODIFY TTL`), so changing the value updates existing tables — no manual ALTER needed. The Job sets `materialize_ttl_after_modify = 0`, so the change is metadata-only: *raising* the TTL takes effect immediately, while *lowering* it purges newly-expired rows lazily on the next background merge rather than at once. To force an immediate purge after lowering, run `ALTER TABLE otel_traces.<table> MATERIALIZE TTL` per affected table. The `llm_*` worker queue tables (`llm_inputs`, `llm_results`, `llm_batches`) are LLM-pipeline state rather than telemetry and are **not** governed by this value — they keep a fixed 30-day TTL defined in their SQL. `llm_worker_info` and `conversation_rollup_watermarks` are current-state control metadata and carry no TTL at all.

> **Operating `conversation_rollup_watermarks`.** The table holds one completeness cursor per `service_name`, published by the conversation turn rollup writer. Two operator facts follow from its engine (`ReplicatedReplacingMergeTree` versioned on the cursor itself). A cursor that has run ahead of what was actually written cannot be corrected by re-publishing a lower one — a lower cursor loses both the read and the merge by design — so correcting one means pausing the writer, then deleting that `service_name`'s rows and confirming the table is empty before resuming it. The delete is an `ALTER DELETE`, which only `schema_owner` holds (the writer's `monte_carlo` does not), and on a replicated table the mutation is asynchronous — resuming before `confirm empty` lets the writer re-read the stale cursor. A second way to reach that state without a writer bug is a `service_name` retired and later reused: the new agent inherits the retired one's cursor, and until corrected every publish regresses, so `max(published_at)` reads stale while the writer is healthy. Finally, a reader that takes the cursor from one replica and the turns from another can see a current cursor over turns that replica has not yet received, which over-asserts completeness; such a reader needs a pinned replica on the turns side. Reading the cursor alone needs no pin, because replica lag can only under-assert. This note is the customer-facing extract; the writer-side rationale lives in the monolith's conversation-materialization readme.
...
| `monte_carlo` | reader bundle¹ | `INSERT` on `llm_inputs`/`llm_batches`/`conversation_eval_scores`/`span_eval_scores`/`conversation_cluster_assignments`/`conversations_normalized`/`conversation_rollup_watermarks` | Monte Carlo (data-source monitoring + agent observability) | always |
