...
> **Telemetry retention** is controlled by `clickhouse.ttlDays` (default 30 days), covering the raw traces, the trace-id timestamp index, the normalized spans, the conversation turns (`conversations_normalized`), and their derived annotations (`conversation_eval_scores`, `span_eval_scores`, `conversation_cluster_assignments`). Unlike the system-log TTLs above, the schema migration Job re-applies this on every install and upgrade (`ALTER TABLE … MODIFY TTL`), so changing the value updates existing tables — no manual ALTER needed. The Job sets `materialize_ttl_after_modify = 0`, so the change is metadata-only: *raising* the TTL takes effect immediately, while *lowering* it purges newly-expired rows lazily on the next background merge rather than at once. To force an immediate purge after lowering, run `ALTER TABLE otel_traces.<table> MATERIALIZE TTL` per affected table. The `llm_*` worker queue tables (`llm_inputs`, `llm_results`, `llm_batches`) are LLM-pipeline state rather than telemetry and are **not** governed by this value — they keep a fixed 30-day TTL defined in their SQL. `llm_worker_info` and `conversation_rollup_watermarks` are current-state control metadata and carry no TTL at all.

> **Operating `conversation_rollup_watermarks`.** The table holds one completeness cursor per `service_name`, published by the conversation turn rollup writer. Two operator facts follow from its engine (`ReplicatedReplacingMergeTree` versioned on the cursor itself). A cursor that has run ahead of what was actually written cannot be corrected by re-publishing a lower one — a lower cursor loses both the read and the merge by design — so correcting one means pausing the writer, deleting that `service_name`'s rows, and confirming the delete landed everywhere before resuming it. Pausing the writer is a Monte Carlo-side action, not a chart knob: the writer is Monte Carlo's scheduled job and reaches this cluster through the `monte_carlo` credential this chart provisions, so coordinate the pause with Monte Carlo before deleting. The delete is an `ALTER DELETE` and the confirmations read `system.mutations` and go through `clusterAllReplicas` (`READ ON REMOTE`) — no single service user holds that combination by design, so the correction runs as the break-glass `admin` user: set `clickhouse.admin.enabled=true`, then `kubectl exec` into the ClickHouse pod (admin is loopback-only by default) and run the statements there. The mutation is asynchronous and completes per replica, so both confirmations go through `clusterAllReplicas`:
>
> ```sql
> -- the mutation finished on every replica (expect no rows):
> SELECT hostName(), count() FROM clusterAllReplicas('otel', system.mutations)
> WHERE table = 'conversation_rollup_watermarks' AND NOT is_done;
>
> -- the service's rows are gone on every replica (expect 0 everywhere):
> SELECT hostName(), count()
> FROM clusterAllReplicas('otel', otel_traces.conversation_rollup_watermarks)
> WHERE service_name = '<svc>';
> ```
>
> Resuming early lets the writer re-read the stale cursor on a replica where the delete has not landed and re-publish it — the correction silently undone. A second way to reach that state without a writer bug is a `service_name` retired and later reused: the new agent inherits the retired one's cursor, and until corrected every publish regresses, so `max(published_at)` reads stale while the writer is healthy. Finally, the cursor and the turns are separate tables with independent replication queues, and a follower fetches parts concurrently — the small cursor part routinely lands ahead of a run's turns part — so on any replica the cursor can transiently assert completeness over turns not yet visible there. A reader that needs the exact guarantee does not infer it from a timestamp: it reads the cursor locally, then issues `SYSTEM SYNC REPLICA otel_traces.conversations_normalized LIGHTWEIGHT` (table-grantable), then reads the turns locally. The writer publishes the cursor only after its turns INSERT lands, so a sync taken after the cursor read waits out exactly those fetches. A wall-clock margin Δ on `now() − published_at` is a fast path, not a substitute. It is a heuristic bound, not a guarantee: the one-row-per-service cursor table replicates near-instantly while the turns table's fetch can lag minutes-to-hours on a replica restart or backlog, and no constant Δ covers that asymmetric case — a Δ gate passes it silently. Δ also cannot stand alone under this engine, because the merge keeps one row per `service_name` and deletes the older rows the gate would otherwise fall back to; at a publish cadence at or below Δ the trusted read finds nothing aged and holds forever. The margin is wall-clock, not the writer's settle window (the event-time lag the writer holds the cursor behind ingest): settle moves the cursor's value in event time but delays nothing about when parts replicate. It must also cover inter-node clock skew, since the comparison runs on the reader's clock against a value stamped on the writer's. `published_at` is the run's wall-clock birth time, evaluated once at insert and replicated with the row. It is provenance, not liveness: an idle run republishing the same watermark is dropped by block dedup and the stamp does not restamp (see 0023's `published_at` note) — liveness comes from the writer's run telemetry. Δ is the reader's constant, not a chart value, so it moves with the reader's release cadence rather than the install's upgrade cadence. "No row" keeps its meaning under this read — never published — but a service whose rows are all younger than Δ is held, not absent, and a reader must not collapse the two:
>
> ```sql
> SELECT service_name,
>        max(watermark)                                                        AS newest,
>        maxOrNullIf(watermark, published_at <= now64(9) - INTERVAL 60 SECOND) AS trusted
> FROM otel_traces.conversation_rollup_watermarks
> GROUP BY service_name
> -- service absent from the result -> never published: the reader starts from its own default floor
> -- trusted IS NULL                -> published but younger than the margin: hold and re-read after it
> -- else                           -> trusted is the cursor to use
> -- (a regressed publisher passes as trusted here — that state shows in max(published_at) reading
> --  stale, which this query does not select)
> -- (60s stands in for Δ, which the reader sizes from measured turns-part fetch times;
> --  `maxOrNullIf`, not `maxIf`: watermark is non-Nullable, so a filtered maxIf returns the
> --  epoch, not NULL, and the `trusted IS NULL` branch would never fire)
> ```
>
...
- **Voter count should be odd** (Raft quorum) and capped at the number of AZs with
...
⁴ **`READ ON REMOTE`** — gates `clusterAllReplicas`, which the rollup writer's correctness-bearing
reads (the ledger anti-join, the cursor read, the duplicate assertion) go through. It takes no
object clause — the `ON *.*` form is a syntax error on this ClickHouse line — so it is global by
construction and the broadest grant in the release; the per-caller network ACL in `values.yaml` is
...
only derived annotations. A stray write from a leaked or misconfigured `monte_carlo` credential is
indistinguishable from a written turn except by its `written_by` marker, and it is not correctable
by re-writing: the table's engine has no version column, and the user holds no `ALTER` or `DROP`.
Correction runs as a user with `ALTER` (see the table above) via `ALTER … DELETE`, or the rows age
out with the TTL. That widening is a reason to tighten `clickhouse.monteCarlo.networksIp` off its
open default on real installs.
...
|-------|---------|-------------|
| `clickhouse.replicasCount` | `2` | Number of ClickHouse replicas in the single-shard cluster. Default is the HA shape (hard per-zone spread; also sets `minDomains` on the zone constraint, so this value is a ceiling — it must not exceed the number of availability zones with eligible nodes, or the excess replicas stay `Pending` permanently instead of co-locating); set `1` for dev/single-AZ installs. On installs with pre-existing plain-`MergeTree` data, hold at `1` until the in-place conversion has verified — see [ClickHouse replication and Keeper](#clickhouse-replication-and-keeper). |
| `clickhouse.storageSize` | `100Gi` | PVC size for ClickHouse data. |
| `clickhouse.ttlDays` | `30` | Retention in days for the telemetry tables (raw traces, trace-id index, normalized spans, conversation turns) and their derived annotations (`conversation_eval_scores`, `span_eval_scores`, `conversation_cluster_assignments`). Re-applied on every install/upgrade via `ALTER TABLE … MODIFY TTL`. Does **not** govern the `llm_*` worker queue tables (they keep a fixed TTL) or the no-TTL control-state tables (`llm_worker_info`, `conversation_rollup_watermarks`). See the telemetry-retention note above. |
